...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Chaîne THÉMATIQUE : on accepte un pool court (≥ HOL_THEMED_MIN_START) plutôt
    // qu'un 503 — l'épuisement déclenche l'écran de victoire. settings.theme + HoL
    // implique supportsChain (sanitizeSettings force budget_guess sinon), donc ce
    // filet ne couvre que l'aléa de session (exclusions, budgets manquants).
